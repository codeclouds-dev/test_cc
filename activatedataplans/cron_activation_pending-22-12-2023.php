...
$CosmoNetworkApi=new CosmoNetworkApi();
 ...
 $query="select user_mstr.id as user_id,email,fullName,user_mstrr.createdDate,pending.id as actv_id,device_details_new.id as device_id,device_details_new.imei,device_details_new.sim_no,address_id,phone_number,provider_phone_number,device_details_new.refurbed,product from pending join device_details_new on pending.device_id=device_details_new.id join user_mstr on user_mstr.id=device_details_new.user_id where pending.status=1   and api='activation pending' order by pending.id desc limit 1";
$arr=array();
$arr['query']=$query;
						...
						//	$sip_res=sipConnectionCreate($get_actv_pending[0]['imei'],$state,$city,$phone,$get_actv_pending[0]['sim_no'],"optimized",$get_actv_pending[0]['device_id'],$get_actv_pending[0]['email']);
							...
							if(is_array($sip_res))
							{
									if($sip_res['response']==1)
									{
											...
											$response_mail=$mail->sendConfirmationMailNew($get_actv_pending[0]['email'],$plan_array[$shopify_product_id],$watch_phone_number,$get_actv_pending[0]['refurbed']);
