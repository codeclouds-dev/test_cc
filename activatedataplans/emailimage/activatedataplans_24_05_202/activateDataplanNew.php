							    	...
							    	else
							    	{
						    			...
						    			if(isset($gigs_useraddrdtls['object']) and $gigs_useraddrdtls['object']=='error' and $gigs_useraddrdtls['message']=='Address already exists.')
						    			{
						    				...
						    				foreach($listuseraddr['items'] as $val)
						    				{
						    					...
						    					if($val['line1']==strtolower($addr1) and $val['line2']==strtolower($addr2) and $val['postalCode']==$postalcode)
						    					{ 
						    			...
						    			// $response=$mail->sendConfirmationMail($getUserDtls[0]['email'],$shopify_order_number,$plan_array[$product_id]);
						    			// $response=$mail->sendMail($getUserDtls[0]['email']);
