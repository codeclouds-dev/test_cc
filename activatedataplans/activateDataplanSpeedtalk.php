...

{
	...
	{	
		...
		$first_name=$order_dtl['orders'][0]['first_name'];
	...
	if($product_id and $type=='CHECKOUT')
		{
			...
			if(count($getUserDtls)>0)
			{
					...
					$arr_addr=array();
					$arr_addr['table']=DEVICETABLE;
					$arr_addr['selector']="id";
					$arr_addr['condition']="where address_id='".$address_id."'";
					$getAddrDtlscheck=$db->Select($arr_addr);

					if(count($getAddrDtlscheck)==0)
					{
								$db->Update(DEVICETABLE,array('address_id'=>$address_id,'status'=>'5',"shopify_product_id"=>$product_id,'payment_date'=>date('Y-m-d H:i:s'),'shopify_order_number'=>$shopify_order_number),array("where user_id='".$getUserDtls[0]['id']."' and status='1'"));
          ...
          $getactvpending=$db->Select($actv_pend);
...

					{
						...
						//$plan_array=array("7664495755485"=>"1 Month Dataplan","7664495198429"=>"6 Month Extended Dataplan","7664492970205"=>"12 Month Annual Dataplan");
						 ...
						 // $plan_array=array("7664495755485"=>"COSMO Monthly Membership Service Plan","7664495198429"=>"6 Month Extended Dataplan","7664492970205"=>"12 Month Annual Dataplan","8054644113629"=>"1 Year Prepaid Plan","8054650699997"=>"2 Year Prepaid Plan","8075971166429"=>"2 Year Prepaid Plan","8074743152861"=>"COSMO Membership - 2 Year Contract");
 ...
 $plan_array=array("7664495755485"=>"COSMO Monthly Membership Service Plan","7664495198429"=>"6 Month Extended Dataplan","7664492970205"=>"12 Month Annual Dataplan","8054644113629"=>"1 Year Prepaid Plan","8054650699997"=>"2 Year Prepaid Plan","8075971166429"=>"2 Year Prepaid Plan","8074743152861"=>"COSMO Membership - 1 Year Contract","8173820575965"=>"1 Year Plan (Monthly)","8199866679517"=>"Monthly Membership - No Contract","8271818096861"=>"Monthly Plan","8271832711389"=>"6 Month Prepaid Plan");
...

				    {
				    	...
				    	//echo "string if ";
							...
							$city=str_replace("'","",$order_dtl['orders'][0]['shipping_address']['city']);
...

						    {
							    ...
							    $device_arr['selector']="id,imei,referrer,phone_number,sim_no,provider,reg_code,telnyx_tag,refurbed,product,channel";

							    //$device_arr['condition']="where user_id='".$getUserDtls[0]['id']."' and status in ('1','5')";

							    $device_arr['condition']="where user_id='".$getUserDtls[0]['id']."' and status='5' and address_id='".$address_id."'";
...

							                              {
							                              	...
							                              	// $db->update(DEVICETABLE,array('provider_phone_number'=>$watch_phone_number_telnyx),array("where id='".$getDeviceDtls[0]['id']."'"));
																							...
																							$sip_array=array("imei"=>$getDeviceDtls[0]['imei'],"sim_no"=>$getDeviceDtls[0]['sim_no'],"device_id"=>$getDeviceDtls[0]['id'],"network_type"=>"optimized");
																								...
																								$sip_addr_array=array("state"=>$state,"city"=>$city,"phone"=>$phone,"email"=>$getUserDtls[0]['email'],"fullName"=>$fullname,"created_at"=>$getUserDtls[0]['createdDate'],"addressCreatedAt"=>date('Y-m-d H:i:s'),"line1"=>$addr1,"line2"=>$addr2,"postalCode"=>$postalcode,"country"=>$country);



																	    				//	$sip_res=sipConnectionCreate($getDeviceDtls[0]['imei'],$state,$city,$phone,$getDeviceDtls[0]['sim_no'],"optimized",$getDeviceDtls[0]['id'],$getUserDtls[0]['email']);
	...
	$sip_array=array("imei"=>$getDeviceDtls[0]['imei'],"sim_no"=>$getDeviceDtls[0]['sim_no'],"device_id"=>$getDeviceDtls[0]['id'],"network_type"=>"optimized");
...

						    			{
				    						...
				    						$json_arr=array("response"=>true,"provider"=>strtolower($getDeviceDtls[0]['provider']),"phone"=>$watch_phone_number,"encrypted_phone"=>$encrypted_phone,"email"=>$email,"encrypted_email"=>$encrypted_email,"referrer"=>$referrer,"imei"=>$encrypted_imei,"uid"=>md5($getUserDtls[0]['id']));
...

{
	...
	$product_type='JT2';
									...
									//	$res_klaviyo1=$klaviyo_api->createMembertoSpeedtalkSubsList($klaviyo1);
						    			...
						    			$res_klaviyo2=$klaviyo_api->createMembertoDataplanSubscriber($klaviyo);
						    			...
						    			$encrypted_order_email=PHP_AES_Cipher::encrypt(KEY,IV,$order_dtl['orders'][0]['email']);
						    			...
						    			$response=$mail->sendConfirmationMailNew($getUserDtls[0]['email'],$plan_array[$product_id],$watch_phone_number,$getDeviceDtls[0]['refurbed'],$encrypted_order_email);
						    			...
						    			$getshopifytags=$ShopifyApi->getOrderDetails($shopify_order_id);
											...
											//print_r($tags_arr);
											$tags_arr=array_merge($tags_arr,array($getDeviceDtls[0]['channel']));
											//print_r($tags_arr);
...

			{
				...
				$json_arr=array("response"=>false,"message"=>"009 | Activation error","block"=>"response_block2");
...

		{
			...
			$json_arr=array("response"=>false,"message"=>"003 | Something wrong with Charge Id","block"=>"response_block2");
...

{
	...
	$json_arr=array("response"=>false,"message"=>"007 | Activation error","block"=>"response_block2");
