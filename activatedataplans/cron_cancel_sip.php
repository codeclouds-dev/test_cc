...
$CosmoSipApi=new CosmoSipApi();
 ...
 $query="select set_cancelation.*,imei,sim_no,email,device_details_new.id as device_id,network_type,device_details_new.status as device_status from set_cancelation join device_details_new on device_details_new.id=set_cancelation.device_id join user_mstr on user_mstr.id=device_details_new.user_id where set_cancelation.status=0 order by id asc limit 1";
$arr=array();
$arr['query']=$query;
...
if(count($getdevdtls)>0)
{
	...
	$db->update(SETCANCELLATION,array('status'=>$getdevdtls[0]['status']+1,"updated_date"=>date('Y-m-d')),array("where id='".$getdevdtls[0]['id']."'"));
	...
	if($res_auth!=true)
	{
		  ...
		  //print_r($set_cancel);
		  $api_log_arr=array("device_id"=>$getdevdtls[0]['device_id'],"api"=>"cancel sip-".$set_cancel['api'],"response"=>json_encode($set_cancel),'data'=>$set_cancel['payload']);
		  //print_r($api_log_arr);
		  $db->Insert(APILOG,$api_log_arr);
		  ...
		  if($res==true)
		  {
				 ...
				 $actv_pend=array("device_id"=>$getdevdtls[0]['device_id'],"api"=>"cancel sip");
				 //print_r($actv_pend);
				 $db->Insert(PENDING,$actv_pend);
		  	 // echo "string2";
		  		if($getdevdtls[0]['status']==3)
				{
					// $dev_arr=array();
					// $dev_arr['imei']=$getdevdtls[0]['imei'];
					// $dev_arr['iccid']=$getdevdtls[0]['sim_no'];
		  ...
		  {
		  	 //echo "string3";
		  		$db->update(SETCANCELLATION,array('status'=>4,"updated_date"=>date('Y-m-d')),array("where id='".$getdevdtls[0]['id']."'"));
