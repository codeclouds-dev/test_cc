...
<?php 
 ...
 $_SESSION['status']=1;
...

  {
     ...
     {
          $where="  and timestamp(pending.created_date) between '$date_from' and '$date_to' and pending.status=3";
...
$index=(($pn-1) * $limit)+1;  
    ...
    $query="select fullName,phoneNumber,email,imei,sim_no,phone_number,activation_date,api,pending.created_date,pending.updated_date  from pending join device_details_new on pending.device_id=device_details_new.id join user_mstr on device_details_new.user_id=user_mstr.id where 1".$where." order by priority asc ";
