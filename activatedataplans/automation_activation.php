...
echo $query="select rech_address_id_webhookissue_2023_12_16_new.id as rech_id,device_details_new.*,fullName,user_mstr.email,createdDate,phoneNumber FROM `rech_address_id_webhookissue_2023_12_16_new` join device_details_new on device_details_new.address_id=rech_address_id_webhookissue_2023_12_16_new.address_id join user_mstr on user_mstr.id=device_details_new.user_id where device_details_new.status!='2' and rech_address_id_webhookissue_2023_12_16_new.user_id=device_details_new.user_id order by rech_address_id_webhookissue_2023_12_16_new.id asc limit 1";
$arr=array();
$arr['query']=$query;
...
{
   $db->Update('rech_address_id_webhookissue_2023_12_16_new',array('status'=>22,'updated_date'=>date('Y-m-d H:i:s')),array("where id=".$getDeviceDtls[0]['rech_id'].""));
  $product_id=$getDeviceDtls[0]['shopify_product_id'];
          ...
          $plan_array=array("7664495755485"=>"COSMO Monthly Membership Service Plan","7664495198429"=>"6 Month Extended Dataplan","7664492970205"=>"12 Month Annual Dataplan","8054644113629"=>"1 Year Prepaid Plan","8054650699997"=>"2 Year Prepaid Plan","8075971166429"=>"2 Year Prepaid Plan","8074743152861"=>"COSMO Membership - 1 Year Contract","8173820575965"=>"1 Year Plan (Monthly)","8199866679517"=>"Monthly Membership - No Contract","8271818096861"=>"Monthly Plan","8271832711389"=>"6 Month Prepaid Plan");
              ...
              //echo "string if ";
    $payment_date=$order_dtl['orders'][0]['created_at'];
                    ...
                    if($addr2=="")
                    {
                      ...
                      if($getDeviceDtls[0]['phone_number']=="" or $getDeviceDtls[0]['phone_number']==NULL  or $getDeviceDtls[0]['phone_number']==0)
                      {
                        ...
                        if(strtolower($getDeviceDtls[0]['provider'])=='telnyx')
                        {
                              ...
                              $response=$TelnyxApi->getSimCardDtls($getDeviceDtls[0]['sim_no']);
                                            ...
                                            else
                                            {
                                              ...
                                              $db->update(DEVICETABLE,array('provider_phone_number'=>$watch_phone_number_telnyx,'activation_date'=>$activation_date),array("where id='".$getDeviceDtls[0]['id']."'"));
                                               ...
                                               // $sip_res=sipConnectionCreate($getDeviceDtls[0]['imei'],$state,$city,$phone,$getDeviceDtls[0]['sim_no'],"optimized",$getDeviceDtls[0]['id'],$getDeviceDtls[0]['email']);
                                                ...
                                                $sip_addr_array=array("state"=>$state,"city"=>$city,"phone"=>$phone,"email"=>$getDeviceDtls[0]['email'],"fullName"=>$fullname,"created_at"=>$getDeviceDtls[0]['createdDate'],"addressCreatedAt"=>date('Y-m-d H:i:s'),"line1"=>$addr1,"line2"=>$addr2,"postalCode"=>$postalcode,"country"=>$country);
  ...
  $sip_array=array("imei"=>$getDeviceDtls[0]['imei'],"sim_no"=>$getDeviceDtls[0]['sim_no'],"device_id"=>$getDeviceDtls[0]['id'],"network_type"=>"optimized");
                    ...
                    /*  else
                      ...
                      $db->Update(DEVICETABLE,array('status'=>2,'shopify_order_number'=>$shopify_order_number,'activation_date'=>$activation_date,'payment_date'=>$payment_date),array("where id='".$getDeviceDtls[0]['id']."'"));

                      $db->Update(USERTABLE,array('status'=>2),array("where id='".$getDeviceDtls[0]['id']."'"));
                        $db->Update('rech_address_id_webhookissue_2023_12_16_new',array('status'=>20,'updated_date'=>date('Y-m-d H:i:s')),array("where id=".$getDeviceDtls[0]['rech_id'].""));
  ...
  $product_type='JT2';
                  ...
                  else
                  {
                    ...
                    //print_r($res_klaviyo1);
                      ...
                      $json_arr=array("response"=>true,"provider"=>strtolower($getDeviceDtls[0]['provider']),"phone"=>$watch_phone_number,"encrypted_phone"=>$encrypted_phone,"email"=>$email,"encrypted_email"=>$encrypted_email,"imei"=>$encrypted_imei,"uid"=>md5($getDeviceDtls[0]['id']));
                    }
                    else
                    {
                            $db->Update('rech_address_id_webhookissue_2023_12_16_new',array('status'=>21,'updated_date'=>date('Y-m-d H:i:s')),array("where id=".$getDeviceDtls[0]['rech_id'].""));
                    }
