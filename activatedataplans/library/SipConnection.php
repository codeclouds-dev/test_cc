...
function sipConnectionCreate($sip_array,$sip_addr_array)
{
			$db=new Database();
			$CosmoSipApi=new CosmoSipApi();
			
				$imei=$sip_array['imei'];
			$iccid=$sip_array['sim_no'];
			$network_type=$sip_array['network_type'];
			$device_id=$sip_array['device_id'];
			$email=$sip_addr_array['email'];
		  ...
		  $db->Insert(APILOG,$api_log_arr);
			...
			{
				//	$res_cosmo=$CosmoSipApi->generateSip(array("imei"=>$imei,"address"=>array("phone_number"=>$phone,"locality"=>$city,"administrative_area"=>$state)),$get_authtoken['accessToken']);

															$res_cosmo=$CosmoSipApi->generateSip(array("imei"=>$imei,"address"=>$sip_addr_array),$get_authtoken['accessToken']);
...
function setActivation($arr)
{
	  ...
	  $res_auth=checkNetworkApiResponse($get_authtoken_network,$dev_arr);
		if($res_auth!=true)
		{
			$setactivation=$CosmoNetworkApi->setActivation(array("imei"=>$arr['imei'],"network"=>$arr['network'],"activation"=>true),$get_authtoken_network['accessToken']);
