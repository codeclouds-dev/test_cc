...
class SpeedtalkApi
{
	...
	//	$endpoint='?cmd=stActivate&agid='.AGID.'&agpass='.AGPASS.'&sku='.SKU.'&amount='.AMOUNT.'&sim='.$simcard.'&firstName='.$data['fname'].'-'.$data['lname'].'&lastName='.$data['email_addr'].'&zip='.ZIPCODE.'&city='.'LAMESA'.'&state='.STATE.'&phone='.PHONE.'&email='.EMAIL.'&campaign='.CAMPAIGN.'&address1='.$data['email_addr'].'&cxemail='.$data['cxemail'].'&address2="'.ADDRESS2.'"';

		

		$endpoint='?cmd=stActivate&agid='.AGID.'&agpass='.AGPASS.'&sku='.SKU.'&amount='.AMOUNT.'&sim='.$simcard.'&firstName='.$data['fname'].'&lastName='.$data['lname'].'&zip='.$data['zip'].'&city='.$data['city'].'&state='.$data['state'].'&Cxcontactno='.$data['phone'].'&email='.EMAIL.'&campaign='.CAMPAIGN.'&address1='.$data['address1'].'&cxemail='.$data['cxemail'].'&address2="'.ADDRESS2.'"';
	...
	public function stAutorefillDeactivate($phone)
	{
		...
		$endpoint='?cmd=stAutorefill&agid='.AGID.'&agpass='.AGPASS.'&phone='.$phone.'&overage=0&overageCash=5&anniversary=0&sku='.SKU;
