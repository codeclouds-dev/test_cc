...
$activated =0;
$mail=0;
echo $query="select refurbed_activation.id as refurb_id,refurbed_activation.shopify_order_id,device_details_new.id,device_details_new.refurbed,device_details_new.shopify_product_id,device_details_new.user_id ,device_details_new.address_id,imei,sim_no,provider,reg_code,email,fullName,phoneNumber,referrer,phone_number,telnyx_tag FROM `refurbed_activation` join device_details_new on refurbed_activation.address_id=device_details_new.address_id JOIN user_mstr on user_mstr.id=refurbed_activation.user_id where device_details_new.address_id!='' and device_details_new.status='5' and refurbed_activation.status=11 and order_number in(188083,190134,190193,190687,191971,192462)";
//167930,167929,174325 already activated
$arr=array();
$arr['query']=$query;
              ...
              $addr2=str_replace("'", "", $addr2);
                  ...
                  //  $referrer=$getDeviceDtls[0]['referrer'];
                    ...
                    if($addr2=="")
                    {
                      ...
                      {
                        $user_addr_ldb=array("line1"=>$addr1,"line2"=>$addr2,"city"=>$city,"state"=>$state,"postalCode"=>$postalcode,"country"=>$country,"user_id"=>$getDeviceDtls[0]['id']);
