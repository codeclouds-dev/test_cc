...
<head>
  ...
  <style>
    ...
    .table>tbody>tr>td, .table>tbody>tr>th, .table>tfoot>tr>td, .table>tfoot>tr>th, .table>thead>tr>td, .table>thead>tr>th {
        padding: 15px 6px;font-size: 13px
    }
    ...
    @media (max-width: 1400px){
    	 .table>tbody>tr>td, .table>tbody>tr>th, .table>tfoot>tr>td, .table>tfoot>tr>th, .table>thead>tr>td, .table>thead>tr>th {
        padding: 15px 3px;font-size: 11px
    }
    }
...
<div class="row">
    ...
    <div class="col-md-12 col-12">
      <form method="post" class="cust-form">
        <div class="col-md-12 col-12" style="padding-bottom: 10px;">
          <div class="col-md-3 col-12 pl-0 mb-3">From Date: <input type="date" name="from_date" id="from_date" value="<?=isset($_POST['from_date'])?$_POST['from_date']:date('Y-m-d')?>" onchange="changedate()"></div>
          <div class="col-md-3 col-12 mb-3">To Date: <input type="date" name="to_date" id="to_date" value="<?=isset($_POST['to_date'])?$_POST['to_date']:date('Y-m-d')?>" onchange="changedate()"></div>

          <div class="col-md-3 col-12 mb-3">Email: 
            <input type="text" name="emailId" id="emailId" value="<?=isset($_POST['emailId'])?$_POST['emailId']:''?>" placeholder="Enter Email ID">
          </div>

          <div class="col-md-3 col-12 mb-3">IMEI: 
            <input type="text" name="imei" id="imei" value="<?=isset($_POST['imei'])?$_POST['imei']:''?>" placeholder="Enter IMEI">
          </div>
        </div>

        <div class="col-md-12 col-12 mb-3">
          <div class="col-md-3 col-12">Sim No: 
            <input type="text" name="iccid" id="iccid" value="<?=isset($_POST['iccid'])?$_POST['iccid']:''?>" placeholder="Enter Sim No. / ICCID">
          </div>

          <div class="col-md-3 col-12 mb-3">Status: 
 ...
 -->
    ...
    <div class="col-12 text-right ext-mrg">
...
<?php 
 ...
 function UTCtoLocal ($dateTime){
  ...
  if(isset($_POST['submit']))
  {
      ...
      $iccid=isset($_POST['iccid'])?$_POST['iccid']:'';
      ...
      if( (isset($emailId)) && (!empty($emailId)) ){
          $where_status .= " and user_mstr.email='$emailId' ";
      }

      if( (isset($imei)) && (!empty($imei)) ){
          $where_status .= " and device_details_new.imei='$imei' ";
      }

      if( (isset($iccid)) && (!empty($iccid)) ){
          $where_status .= " and device_details_new.sim_no='$iccid' ";
      }
  ...
  else if( (isset($emailId)) && (!empty($emailId)) )
  {
      $where=$_SESSION['where_status_dev'];
  }
  else if( (isset($imei)) && (!empty($imei)) )
  {
      $where=$_SESSION['where_status_dev'];
  }
  else if( (isset($iccid)) && (!empty($iccid)) )
  {
      $where=$_SESSION['where_status_dev'];
  }
  else
  {
    ...
    $query="select fullName,phoneNumber,email,imei,sim_no,phone_number,activation_date,updated_date,provider,product,fw_version  from device_details_new join user_mstr on device_details_new.user_id=user_mstr.id where 1".$where." order by activation_date desc";
...
document.write(MyTimeZone);
    ...
    <tbody>
      ...
      <tr>
        ...
        <td><?=$val['imei'];?></td>
        <td><?=$val['sim_no'];?></td>
        <td><?=ucfirst($val['provider']);?></td>
                <td><?=$val['product'];?></td>
        <td><?=$val['fw_version'];?></td>
