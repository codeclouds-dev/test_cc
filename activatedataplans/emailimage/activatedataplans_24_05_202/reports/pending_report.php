...
<div class="row">
  ...
  <div class="row">
   ...
   <div class="col-md-3 col-12">Status: <select name="status"><option value="0" <?php if(isset($_POST['status']) and $_POST['status']==0){ echo "selected";}?>>In process</option><option value="1" <?php if(isset($_POST['status']) and $_POST['status']==1){ echo "selected";}?>>Failure</option><option value="2" <?php if(isset($_POST['status']) and $_POST['status']==2){ echo "selected";} else if(!isset($_POST['status'])){ echo "selected";}?>>Success</option></select></div>
 ...
 function UTCtoLocal ($dateTime){
...

  {
...

      {
          ...
          $where_status=" and  timestamp(pending.created_date) between '$date_from' and '$date_to' and pending.status='4'";
          $_SESSION['status']=2;
      }
...

      {
          ...
          $where_status="  and timestamp(pending.created_date) between '$date_from' and '$date_to' and pending.status='3'";
 $_SESSION['status']=1;
      }
      else
      {
...

  {
      ...
      $where=$_SESSION['where_status_pen'];
...

  {
      ...
      $where=" and timestamp(pending.created_date) between '$date_from' and '$date_to' and pending.status='4' ";
 $_SESSION['status']=2;
  }
...
$index=(($pn-1) * $limit)+1;  
    ...
    $query="select fullName,phoneNumber,email,imei,sim_no,phone_number,activation_date,api,pending.created_date,pending.updated_date  from pending join device_details_new on pending.device_id=device_details_new.id join user_mstr on device_details_new.user_id=user_mstr.id where 1".$where." order by priority asc limit $limit offset $start_from";
...
document.write(MyTimeZone);
        ...
        <th id="TimeZone">Created Date</th>

         <th id="TimeZone">Updated Date</th>
    ...
    <tbody>
      ...
      <tr>
        ...
        <!-- <td><?=date('Y-m-d',strtotime($val['activation_date']));?></td> -->
         ...
         <td nowrap="nowrap"><?  if($_SESSION['status']==0){ echo "In process"; } elseif ($_SESSION['status']==1) {
         echo  "Failure";
         }else{ echo "Success"; }?></td> 
...
<td rowspan="10" align="center">No record found!</td>
   ...
   <div class="col-md-12">
...

      {
          ...
          $sql = "select count(*) as count FROM pending where 1 ".$where;  
          ...
          if($total_pages>1)
          {
               for ($i=1; $i<=$total_pages; $i++) {
                        ...
                        if ($i==$pn) {
                            ...
                            $pagLink .= "<li class='active'><a href='pending_report.php?page="
                            ...
                            $pagLink .= "<li><a href='pending_report.php?page=".$i."'>
