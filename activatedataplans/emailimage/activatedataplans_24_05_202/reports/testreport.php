 ...
 function UTCtoLocal ($dateTime){
  ...
  if(isset($_POST['submit']))
  {
      ...
      if($status==2)
      {
          ...
          $_SESSION['query']="select fullName,phoneNumber,email,imei,sim_no,phone_number,activation_date,updated_date  from device_details_new join user_mstr on device_details_new.user_id=user_mstr.id where 1".$where_status." order by activation_date desc";
      }
      else
      {
          //$where_status=" and device_details_new.status='6' and timestamp(updated_date) between '$date_from' and '$date_to'";
            $where_status=" and timestamp(device_details_new.updated_date) between '$date_from' and '$date_to'";

          $_SESSION['query']="with cancelled as (
   
SELECT imei,sim_no,user_id,phone_number,device_details_new.updated_date,case when device_details_new.status='6' then 'Cancelled' end as status from device_details_new where device_details_new.id not in(SELECT device_id from recharge_cancellation) and device_details_new.status='6' and timestamp(device_details_new.updated_date) between '$date_from' and '$date_to'           
UNION
select imei,sim_no,user_id,phone_number,recharge_cancellation.updated_date,case when recharge_cancellation.status='6' then 'Cancelled' when recharge_cancellation.status='7' then 'Fixed' when recharge_cancellation.status='8' then 'Error' when recharge_cancellation.status>0 and recharge_cancellation.status<6 then 'Pending' end as status from recharge_cancellation join device_details_new on device_details_new.id=recharge_cancellation.device_id and timestamp(recharge_cancellation.updated_date) between '$date_from' and '$date_to'

) select fullName,email,phoneNumber,imei,sim_no,cancelled.status,updated_date,phone_number from cancelled join user_mstr on user_mstr.id=cancelled.user_id";
  ...
  else
  {
    ...
    //  $where=" and timestamp(activation_date) between '$date_from' and '$date_to' and device_details_new.status='2'";
       $_SESSION['query']="select fullName,phoneNumber,email,imei,sim_no,phone_number,activation_date,updated_date  from device_details_new join user_mstr on device_details_new.user_id=user_mstr.id where 1 and timestamp(activation_date) between '$date_from' and '$date_to' and device_details_new.status='2' order by activation_date desc";
  }
