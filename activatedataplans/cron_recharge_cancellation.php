...
{
   echo  $query="select recharge_cancellation.id,device_details_new.phone_number,provider_phone_number,address_id,network_type,recharge_cancellation.status,device_details_new.id as device_id,imei,sim_no,email,provider from recharge_cancellation join device_details_new on device_details_new.id=recharge_cancellation.device_id join user_mstr on user_mstr.id=device_details_new.user_id where recharge_cancellation.status=0 order by recharge_cancellation.id asc limit 1";
...
echo "string";
                         if(strtolower($recharge_cancellation[$i]['provider'])=='speedtalk')
                         {
                              ...
                              if($res['ret']==0)
                              {
                                          ...
                                          if(strrpos($recharge_cancellation[$i]['imei'],'-duplicate'))
                              {
                                     echo "string1";

                                     $check_imei_arr['table']='imei_duplicate_cancel_req';
                                        $check_imei_arr['selector']='id';
                                        $check_imei_arr['condition']="where device_id='".$recharge_cancellation[$i]['device_id']."'";
                                        
                                        $checkIMEIExists=$db->Select($check_imei_arr);
                                        if(count($checkIMEIExists)==0)
                                        {

                                            $api_log_arr=array("device_id"=>$recharge_cancellation[$i]['device_id'],"imei"=>$recharge_cancellation[$i]['imei'],"address_id"=>$recharge_cancellation[$i]['address_id']);
                                                
                                           //print_r($api_log_arr);
                                           $db->Insert("imei_duplicate_cancel_req",$api_log_arr);
                                        }
                                   $db->Update("recharge_cancellation",array('status'=>9,'updated_date'=>$updated_date,"notes"=>"N/A"),array("where id='".$recharge_cancellation[$i]['id']."'"));
                                             ...
                                             else
                                             {
                                                         $res=$CosmoNetworkApi->getAuthToken();
                                                            ...
                                                            $res2=$CosmoNetworkApi->getCurrentNetworkType(array("imei"=>$recharge_cancellation[$i]['imei']),$res['accessToken']);
                                                            //print_r($res2);
                                                             $api_log_arr=array("device_id"=>$recharge_cancellation[0]['device_id'],"api"=>$res2['api'],"response"=>json_encode($res2));
                                                              //print_r($api_log_arr);
                                                              $db->Insert(APILOG,$api_log_arr);
                                                            $network=$res2['current_call_network'];
                                                            $db->Update("device_details_new",array('current_call_network_cosmo'=>$network),array("where id='".$recharge_cancellation[$i]['device_id']."'"));
                                                             $res2=$CosmoSipApi->checkSIP(array("imei"=>$recharge_cancellation[$i]['imei']),$res['accessToken']);
                                                          ...
                                                          if( $sip==1)
                                                            {
                                                               $check_set_cancel_arr['table']=SETCANCELLATION;
                                                                    $check_set_cancel_arr['selector']='id';
                                                                    $check_set_cancel_arr['condition']="where device_id='".$recharge_cancellation[$i]['device_id']."'";
                                                                    
                                                                    $check_set_cancel_exist=$db->Select($check_set_cancel_arr);
                                                                    if(count($check_set_cancel_exist)==0)
                                                                    {
                                                                       $set_cancellation_arr=array("device_id"=>$recharge_cancellation[$i]['device_id']);
                                                                       //print_r($api_log_arr);
...
echo $recharge_cancellation[0]['imei'];
                                        if(strrpos($recharge_cancellation[$i]['imei'],'-duplicate'))
                              {
                                     echo "string1";
                                     $check_imei_arr['table']='imei_duplicate_cancel_req';
