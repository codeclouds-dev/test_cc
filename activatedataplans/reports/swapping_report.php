...
<html lang="en">
<head>
  <title>Device Swapping Report</title>
  <meta charset="utf-8">
...
<?php 
 ...
 function UTCtoLocal ($dateTime){
  ...
  else
  {
    ...
    $query="select *,device_swapping.status as swap_status, case when device_swapping.status=4 then 'Activated' else 'Pending' end as status_text from device_swapping join device_details_new on device_details_new.id=device_swapping.new_device_id left join user_mstr on user_mstr.id=device_details_new.user_id where 1 ".$where;
...
if(isset($_POST['export']))
 {
    ...
    header('Content-Disposition: attachment; filename=Device Swapping Report.xls');
   ...
   <div class="col-md-12 scroll-page">
    ...
    <tbody>
      <?php 
        ...
        {
            $status=$val['swap_status'];

            $olddevice=array();
            $olddevice['table']=DEVICETABLE;
            $olddevice['selector']="*";
            $olddevice['condition']="where id='".$val['old_device_id']."'";
            $getolddevice=$db->Select($olddevice);

            if(count($getolddevice)>0)
            {
                $old_device=$val['product'];
            }

            $newdevice=array();
            $newdevice['table']=DEVICETABLE;
            $newdevice['selector']="id";
            $newdevice['condition']="where id='".$val['new_device_id']."'";
            $getnewdevice=$db->Select($newdevice);

            if(count($getnewdevice)>0)
            {
                $new_device=$val['product'];
            }

            if($val['swap_status']==4)
            {
              $status='Activated';

            }
            else if($val['swap_status']==0)
            {
                  $status='Pending';
                  $rech_cancel=array();
                  $rech_cancel['table']='recharge_cancellation';
                  $rech_cancel['selector']="*";
                  $rech_cancel['condition']="where device_id='".$val['old_device_id']."'";
                  $getcancel=$db->Select($rech_cancel);
                  if(count($getcancel)>0)
                  {
                      if(strtolower($getolddevice[0]['provider'])=='speedtalk' and $getolddevice[0]['network_type']=='standard')
                      {

                        if($getcancel[0]['status']==6 or $getcancel[0]['status']==7) 
                        {
                      ...
                      else if(strtolower($getolddevice[0]['provider'])=='telnyx' or (strtolower($getolddevice[0]['provider'])=='speedtalk' and $getolddevice[0]['network_type']=='optimized'))
                      {
                          $sip_status=array();
                          $sip_status['table']='set_cancelation';
                          $sip_status['selector']="*";
                          $sip_status['condition']="where device_id='".$val['old_device_id']."'";
                          $getsipstatus=$db->Select($sip_status);
                          if(count($getsipstatus)>0)
                          {
                            if($getsipstatus[0]['status']==4)
                            {
                                  $status='Cancelled Successfully!';
      ...
      <tr>
        ...
        <td><?=$val['phone_number'];?></td>
         ...
         <td nowrap="nowrap" class="dateTime"><?php echo $val['created_date']; ?></td> 
         <td nowrap="nowrap" ><?php echo $val['created_date']; ?></td> 

         <td nowrap="nowrap" ><?php echo $old_device; ?></td> 
         <td nowrap="nowrap" ><?php echo $new_device ?></td> 
...
function changedate()
{
 ...
 document.getElementById("export").addEventListener("click", function () {
  ...
  htmlToCSV(html, "Device Swapping Report.csv");
});
