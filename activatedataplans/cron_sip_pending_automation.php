...
else
{
    $product_type='JT2';
              ...
              //$res_klaviyo1=$klaviyo_api->createMembertoSpeedtalkSubsList($klaviyo1);
               ...
               $encrypted_order_email=PHP_AES_Cipher::encrypt(KEY,IV,$getdevdtls[0]['email']);
               $response=$mail->sendConfirmationMailNew($getdevdtls[0]['email'],$plan_array[$product_id],$watch_phone_number,$getdevdtls[0]['provider'],$encrypted_order_email);
               //print_r($response);
            ...
            $getshopifytags=$ShopifyApi->getOrderDetailsbyOrderNo($getdevdtls[0]['shopify_order_number']);



            $order_tag=$getshopifytags['orders'][0]['tags'];
            $shopify_order_id=$getshopifytags['orders'][0]['id'];

            $tags_arr=explode(",",$order_tag);
            //print_r($tags_arr);
            $tags_arr=array_merge($tags_arr,array($getdevdtls[0]['channel']));
            //print_r($tags_arr);
